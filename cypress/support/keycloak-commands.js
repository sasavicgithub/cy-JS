...
Cypress.Commands.add('keycloakLogin', (username, password, redirectUrl = Cypress.env('appUrl')) => {
  ...
  return cy.request({
    ...
    if (redirectLocation.includes('code=')) {
      ...
      throw new Error('Login failed - no authorization code in redirect. Redirect URL: ' + redirectLocation);
    }
  });
});

Cypress.Commands.add('keycloakLoginForm', (username, password) => {
...
Cypress.Commands.add('setupAuthenticatedSession', () => {
  ...
  cy.keycloakLogin(username, password, mapUrl).then((tokens) => {
...
Cypress.Commands.add('isAuthenticated', () => {
  return cy.request({
    ...
    return !response.headers.location || !response.headers.location.includes(Cypress.env('authBaseUrl'));
  });
});
